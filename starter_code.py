...
def count_files(directory_path):
    ...
    # The best case for counting files is when the directory_path given is a file itself, in whcih case we can return 1. 
    # However, ff it's a directory, we need to keep drilling down to look for other files.
    # 2. How do you list items in a directory? (Check Resource 3)
    # os.listdir() gives you the names of items in a directory, but you need to join them with the directory path to get all of the possible paths.
    # 3. For each item, is it a file or directory? Recursively handle directories.
    # Use os.path.isfile() and os.path.isdir() to check if it's a file or directory. Files don't need to be iterated any further.
    # 4. How do you combine the results?
    # You can keep a running total of files found in the directory and its subdirectories, and return that total at the end.
...
if __name__ == "__main__":
    ...
    print("Total Infected Files (breached files):", len(find_infected_files("breach_data/Operations"))) # ???

    ## 5. Determine how many files were corrupted by department (Finance, HR, and Sales)
    #Finance: 1149, Marketing/Sales: 1368, Operations/HR: 2612
